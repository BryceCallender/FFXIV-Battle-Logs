...
class _SpecificFightReportState extends State<SpecificFightReport> {
  ...
  Future<List<charts.Series>> getGraphData() async {
    if (hpsData == null && dpsData == null) {
      ...
      for(int i = 0; i < sortedByPartyWidgetDPSInfo.length; i++) {
        sortedByPartyWidgetDPSInfo.elementAt(i).user = dummyNames[i];
      }
      ...
      for(int i = 0; i < sortedByPartyWidgetHPSInfo.length; i++) {
        sortedByPartyWidgetHPSInfo.elementAt(i).user = dummyNames[i];
      }
